...
def test_restaurants(cuisines, driver, engine):
    """"""
    html = driver.page_source
    for c in cuisines:
        click_option(driver, option=c)
        restaurants(cuisine=c, html=html, engine=engine)

        time.sleep(2)
        soups = BeautifulSoup(driver.page_source, "html.parser")
        restaurant_amount = soups.find("span", {"class": "restaurant-amount"}).text

        assert len(pd.read_sql_table(c, con=engine)) == int(restaurant_amount), f"Error occured for {c}"
